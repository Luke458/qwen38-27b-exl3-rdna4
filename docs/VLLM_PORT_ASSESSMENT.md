...
| 5 | 16 GB fit | **done (2026-09-25)** | Model load 11.17 GiB with vision (MTP drafter shares embed/lm_head via placeholders, vision qkv in EXL3). int8 per-token-head KV (KL equal to fp16 KV; per-tensor fp8 rejected, KL 5x worse) with an fp16-dequant prefill path. MTP-3 + vision: 16k context, 30.5k KV tokens, peak 15.95 GiB. Plain: 64k context, 72k KV tokens, peak 15.95 GiB. See `experiments/0022-kv-fp8/RESULTS.md` and `vllm_plugin/README.md`. |
