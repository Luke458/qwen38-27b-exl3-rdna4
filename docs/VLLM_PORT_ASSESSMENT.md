...
| status | **74 tok/s MTP-3, 41 tok/s plain (2026-09-25)** | — | GEMV core rewrite + multi-row verify (fork patches in `vllm_plugin/patches/exl3-fork/`), 1 HW queue, V2 runner, GDN patch. Teacher-forced logits vs fork: top-1 128/128. See `docs/DECODE_TRACE.md`. |
