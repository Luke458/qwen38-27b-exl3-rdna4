...
| 5 | 16 GB fit | **done (2026-09-26)** | Model load 10.86 GiB with MTP and vision (drafter shares embed/lm_head, pruned draft head reads the lm_head in place, vision qkv in EXL3, rotary cache clamped). int8 per-token-head KV (KL equal to fp16 KV; per-tensor fp8 rejected, KL 5x worse) with an fp16-dequant prefill path. MTP-3 + vision at 32k context: server peak 14,755 MiB (40k: 15,122). Plain at 64k: 15,171 MiB. A vLLM KV-headroom stall near max-model-len was found and is guarded. See `experiments/0022-kv-fp8`, `experiments/0023-context` and `vllm_plugin/README.md`. |
